...
router.post("/create-user", upload.single("file"), async (req, res, next) => {
  try {
    ...
    try {
      ...
      // In local/dev, don't block signup when SMTP is not configured.
      if (process.env.NODE_ENV !== "PRODUCTION") {
        return res.status(201).json({
          success: true,
          message:
            "Account created for activation, but email service is not configured. Use activationUrl from this response.",
          activationUrl,
        });
      }
